...
def get_name_namespace_constructors(t, arg, type_constructors):
    ...
    if '.' not in name:
        if constructors:
            if constructors[0].namespace is not None:
...
def write_import_in_method(namespace: str, names: str, builder: SourceBuilder, filename: str):
    if namespace == filename or (not namespace and '__init__' in str(filename)):
        return
    ...
    for arg in tlobject.args:
        if arg.type in BASE_TYPES:
            ...
            if not arg.skip_constructor_id:
                ...
                write_import_in_method(namespace, ', '.join(class_names), builder, filename)
                ...
                name = 'Type' + name
                write_import_in_method(namespace, name, builder, filename)
...
def _write_arg_read_code(builder, arg, tlobject, name, filename: str):
    ...
    if arg.is_vector:
        ...
        if not arg.skip_constructor_id:
            ...
            write_import_in_method(ns, class_name, builder, filename)
