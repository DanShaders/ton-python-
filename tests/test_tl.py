...
from tl.tl_gen.ton_api.types import Pk_aes, Pub_unenc, Id_config_local, Catchain_config_global, Engine_validator_config, Adnl_packetContents
...
@pytest.fixture()
def pubk():
    return Pub_unenc(data_=b'\x12' * 32)


@pytest.fixture()
def block():
    return TonNode_blockIdExt(workchain_=-1, shard_=0, seqno_=123456, root_hash_=b'\x01' * 32, file_hash_=b'\x02' * 32)
...
def test_reading_engine_validator_config():
    ...
    assert vc.adnl[0].id == base64.b64decode(json.loads(config)['adnl'][0]['id'])
...
def test_request(block: TonNode_blockIdExt):
    req = LiteServer_getBlockHeaderRequest(id_=block, mode_=2)
    req2 = LiteServer_getBlockHeaderRequest.from_dict(req.to_dict())
    assert req == req2


def test_binary_reader(block: TonNode_blockIdExt):
    # test serialization and deserialization into same object

    reader = BinaryReader(block.to_bytes()[4:], tl_objects=tlobjects)  # skip tag
    blk2 = TonNode_blockIdExt.from_reader(reader)
    assert blk2 == block

def test_binary_reader_parsing(block: TonNode_blockIdExt):
    # some real examples from liteserver
